...
UNIVERSITIES = [
    # 10 Universitas Terbaik di Dunia
    {
        ...
        "programs": {
            ...
            "S2&S3": {
                "min_ipk": 3.7,
                "min_sat": 1400,  # Mungkin tidak relevan untuk S2&S3
                "min_toefl": 100,
                "min_raport": 80,
                "penelitian": "ya",
                "rekomendasi": "sangat baik",
    ...
    },
    {
        ...
        "programs": {
            ...
            "S2&S3": {
                "min_ipk": 3.6,
                "min_sat": 1350,  # Mungkin tidak relevan untuk S2&S3
                "min_toefl": 95,
                "min_raport": 75,
                "penelitian": "ya",
                "rekomendasi": "baik",
    ...
    # 20 Universitas Terbaik di Indonesia
    {
        ...
        "programs": {
            ...
            "S2&S3": {
                "min_ipk": 3.4,
                "min_sat": 80,  # Mungkin tidak relevan untuk S2&S3
                "min_toefl": 80,
                "min_raport": 70,
                "penelitian": "baik",
                "rekomendasi": "baik",
...
def evaluasi_peluang(user_data, university=None, program_type="S1"):
    skor = 0
    if university:
        # Mengambil kriteria minimal dari universitas yang dipilih berdasarkan program
        program = university["programs"].get(program_type)
        if not program:
            return skor, "Rendah"  # Program tidak tersedia

        min_ipk = program["min_ipk"]
        min_sat = program["min_sat"]
        min_toefl = program["min_toefl"]
        min_raport = program["min_raport"]
        req_penelitian = program["penelitian"]
        req_rekomendasi = program["rekomendasi"]
        req_sop = program["sop"]
        ...
        # Evaluasi Skor Tes Standar (SAT/SNMPTN/SBMPTN)
        if program_type == "S1":
        ...
        # Evaluasi Nilai Rata-rata Raport
        if user_data["raport"] >= min_raport:
            skor += 30
        elif user_data["raport"] >= min_raport - 15:
            skor += 15
        else:
            skor += 5
...
def find_alternative_university(user_data, program_type="S1"):
    # Cari universitas terbaik yang kriteria input pengguna penuhi
    for uni in UNIVERSITIES:
        try:
            program = uni["programs"].get(program_type)
            if not program:
                continue  # Program tidak tersedia
            ...
            # Cek IPK
            if user_data["ipk"] < min_ipk - 0.3:
                continue
            # Cek SAT
            if user_data["sat"] < min_sat - 150:
                continue
            # Cek TOEFL
            if user_data["toefl"] < min_toefl - 20:
                continue
            # Cek Nilai Raport
            if user_data["raport"] < min_raport - 15:
                continue
            # Cek Pengalaman Penelitian
            if req_penelitian == "ya" and user_data["penelitian"] != "ya":
                continue
            elif req_penelitian == "baik" and user_data["penelitian"] not in ["ya", "baik"]:
                continue
            # Cek Surat Rekomendasi
            rekom_levels = ["cukup", "baik", "sangat baik"]
            user_rekom = rekom_levels.index(user_data["rekomendasi"])
            req_rekom = rekom_levels.index(req_rekomendasi)
            if user_rekom < req_rekom -1:
                continue
            # Cek SOP
            sop_levels = ["cukup", "baik", "sangat baik"]
            user_sop = sop_levels.index(user_data["sop"])
            req_sop = sop_levels.index(req_sop)
...
def submit():
    try:
        ...
        if not (0.0 <= ipk <= 4.0):
            ...
            return
        if not (0 <= sat <= 1600):
            messagebox.showerror("Input Error", "Skor tes standar (SAT/SNMPTN/SBMPTN) harus antara 0 dan 1600.")
            return
        if not (0 <= toefl <= 120):
            messagebox.showerror("Input Error", "Skor bahasa (TOEFL) harus antara 0 dan 120.")
            return
        if not (0 <= raport <= 100):
            messagebox.showerror("Input Error", "Nilai rata-rata raport harus antara 0 dan 100.")
            return
        ...
        skor, peluang = evaluasi_peluang(user_data, selected_uni, program)

        result = f"Skor Total: {skor}/180\nPeluang Diterima di {kampus}: {peluang}"

        if peluang != "Tinggi":
            # Cari rekomendasi universitas alternatif
            alternative_uni = find_alternative_university(user_data, program)
...
def update_fields(*args):
    program = var_program.get()
    if program == "S1":
        label_tes.config(text="Skor Tes Standar (SAT/SNMPTN/SBMPTN) 0-1600:")
        entry_tes.config(state="normal")
    else:
        label_tes.config(text="Skor Tes Standar (SAT/SNMPTN/SBMPTN) 0-1600:")
        entry_tes.config(state="normal")
    # Anda bisa menambahkan logika lebih lanjut jika ada perbedaan pertanyaan atau penyesuaian lainnya
...
# Target Program
label_program = tk.Label(frame, text="Target Program:", font=("Helvetica", 12))
label_program.grid(row=0, column=0, sticky="e", padx=5, pady=5)
var_program = tk.StringVar(value="S1")
dropdown_program = tk.OptionMenu(frame, var_program, "S1", "S2&S3", command=update_fields)
dropdown_program.config(width=10)
dropdown_program.grid(row=0, column=1, padx=5, pady=5, sticky="w")

# IPK
label_ipk = tk.Label(frame, text="IPK (0.0 - 4.0):", font=("Helvetica", 12))
label_ipk.grid(row=1, column=0, sticky="e", padx=5, pady=5)
entry_ipk = tk.Entry(frame)
entry_ipk.grid(row=1, column=1, padx=5, pady=5)

# Skor Tes Standar (SAT/SNMPTN/SBMPTN)
label_tes = tk.Label(frame, text="Skor Tes Standar (SAT/SNMPTN/SBMPTN) 0-1600:", font=("Helvetica", 12))
label_tes.grid(row=2, column=0, sticky="e", padx=5, pady=5)
entry_tes = tk.Entry(frame)
entry_tes.grid(row=2, column=1, padx=5, pady=5)

# Skor Bahasa (TOEFL)
label_bahasa = tk.Label(frame, text="Skor Bahasa (TOEFL 0-120):", font=("Helvetica", 12))
label_bahasa.grid(row=3, column=0, sticky="e", padx=5, pady=5)
entry_bahasa = tk.Entry(frame)
entry_bahasa.grid(row=3, column=1, padx=5, pady=5)

# Nilai Rata-rata Raport
label_raport = tk.Label(frame, text="Nilai Rata-rata Raport (0-100):", font=("Helvetica", 12))
label_raport.grid(row=4, column=0, sticky="e", padx=5, pady=5)
entry_raport = tk.Entry(frame)
entry_raport.grid(row=4, column=1, padx=5, pady=5)

# Pengalaman Penelitian
label_penelitian = tk.Label(frame, text="Pengalaman Penelitian:", font=("Helvetica", 12))
label_penelitian.grid(row=5, column=0, sticky="e", padx=5, pady=5)
...
radio_penelitian_ya.grid(row=5, column=1, sticky="w", padx=5)
radio_penelitian_tidak.grid(row=5, column=1, padx=65, sticky="w")

# Surat Rekomendasi
label_rekomendasi = tk.Label(frame, text="Kualitas Surat Rekomendasi:", font=("Helvetica", 12))
label_rekomendasi.grid(row=6, column=0, sticky="e", padx=5, pady=5)
...
dropdown_rekomendasi.grid(row=6, column=1, padx=5, pady=5, sticky="w")

# Pernyataan Tujuan (SOP)
label_sop = tk.Label(frame, text="Kualitas Pernyataan Tujuan (SOP):", font=("Helvetica", 12))
label_sop.grid(row=7, column=0, sticky="e", padx=5, pady=5)
...
dropdown_sop.grid(row=7, column=1, padx=5, pady=5, sticky="w")

# Pilihan Kampus Tujuan
label_kampus = tk.Label(frame, text="Kampus Tujuan:", font=("Helvetica", 12))
label_kampus.grid(row=8, column=0, sticky="e", padx=5, pady=5)
...
dropdown_kampus.grid(row=8, column=1, padx=5, pady=5, sticky="w")
